    ...
    public static class MathTokenizer
    {
        public static IEnumerable<MathToken> Tokenize(string expression)
        {
            ...
            foreach (var part in parts)
            {
                ...
                while (curPos < part.Length)
                {
                    ...
                    {
                        if (part.Length == 1)
                            throw new InvalidSymbolException(ex.Message);
                        throw new InvalidNumberException($"{MathErrorMessager.NotNumberMessage(part)}");
                    }
        ...
        private static MathToken TokenizeOperator(string part, int position)
        {
            return part[position] switch
            {
                ...
                _ => throw new InvalidSymbolException($"{MathErrorMessager.UnknownCharacterMessage(part[position])}")
            };
        }
